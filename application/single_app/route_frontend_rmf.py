...
def register_route_frontend_rmf(bp):
    ...
    @enabled_required("enable_rmf")
    def rmf_chat():
        """Render grounded, read-only investigative chat for the active RMF workspace."""
        user_id = get_current_user_id()
        try:
            group_id = require_active_group(user_id)
        except ValueError:
        ...
        if not rmf_state["enabled"]:
            return redirect(url_for("frontend_rmf.rmf_workspace"))
        return render_template(
            "rmf_chat.html",
            settings=sanitize_settings_for_user(get_settings()),
            group=group_doc,
            group_role=get_user_role_in_group(group_doc, user_id),
            rmf_state=rmf_state,
        )
